namespace SJInovacao.Acesso.Common.Security.Context
{
    public class UserContext : IUserContext
    {
        ...
        public void SetUserData(string? userId, string? userName, string? userRole, IEnumerable<string> permissions, IEnumerable<string> groups)
        {
            UserId = userId;
            UserName = userName;
            UserRole = userRole;
            Permissions = (permissions ?? Enumerable.Empty<string>()).ToList().AsReadOnly();
            Groups = (groups ?? Enumerable.Empty<string>()).ToList().AsReadOnly();
        }
    }
}